  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground sm:text-3xl">
            Olá{stats?.userName ? `, ${stats.userName}` : ""}!
          </h1>
          <p className="mt-1 text-sm text-muted">
            {connectedCount === 1 ? "1 conta ligada" : `${connectedCount} contas ligadas`}
            {" · "}
            {stats?.contactsCount ?? 0}{" "}
            {(stats?.contactsCount ?? 0) === 1 ? "contato" : "contatos"}
            {" · "}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
        ...
        <StatCard label="Ignorados" value={stats?.dmsSkippedMonth ?? 0} />
        ...
        <div className="lg:col-span-1 panel rounded p-4 sm:p-6">
          ...
          <div className="space-y-3">
            {stats?.topKeywords.length === 0 && (
              <p className="text-sm text-muted py-8">Nenhuma palavra-chave ainda</p>
            )}
        ...
        <div className="lg:col-span-2 panel rounded p-4 sm:p-6">
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto">
            {stats?.recentLogs.length === 0 && (
              <p className="text-sm text-muted text-center py-8">Nada ainda</p>
            )}
