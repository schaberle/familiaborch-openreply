      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted mt-1">
            {data.requestedCount === "all" ? "Todo o período" : "Recente"} —{" "}
            {totals.posts} post{totals.posts === 1 ? "" : "s"} de @
            {data.account.username}
            {data.truncated ? ` (até ${totals.posts})` : ""}
          </p>
          {followers !== null && (
      ...
      {!insightsAvailable && (
        <div className="panel rounded p-4 border border-border">
          <p className="text-sm text-foreground">
            Visualizações, alcance, salvos e compartilhamentos pedem a permissão de insights.
          </p>
          <p className="text-sm text-muted mt-1">
