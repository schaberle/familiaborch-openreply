...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (triggerScope === "specific" && !postId)
      return setError("Escolha um post ou reel para a campanha.");
    if (matchMode === "specific" && keywords.length === 0)
      return setError("Coloque ao menos uma palavra-chave, ou use qualquer palavra.");
    ...
    if (openingDmEnabled && (!openingDmMessage.trim() || !openingDmButtonLabel.trim()))
      return setError("O primeiro DM precisa de mensagem e texto do botão.");
      ...
      {importQueue && (
        <div className="rounded border border-accent/30 bg-accent/5 px-4 py-3 text-sm">
          ...
          <span className="text-muted">
            Campos vieram do CSV. Escolha o reel, ajuste o que quiser e salve
            para ir ao próximo — ou Pular se não quiser este.
          </span>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          {importQueue && (
            ...
            >
              {importQueue.length > 1 ? "Pular" : "Pular e terminar"}
            </button>
          )}
          {mode === "edit" &&
            (isActive ? (
          ...
          >
            {saving ? "Salvando…" : mode === "new" ? "Publicar" : "Salvar"}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-8">
        ...
        <Section title="E o comentário tiver">
          ...
          {matchMode === "specific" && (
            <div className="space-y-1">
              <input
                ...
                placeholder="Uma ou várias palavras"
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
              />
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              também responder se mandarem DM com{" "}
          ...
          {dmTriggerEnabled && (
            <p className="text-xs text-muted">
              {matchMode === "any"
                ? "Qualquer DM nesta conta recebe a resposta abaixo — use com cuidado."
                : "Um DM com essas palavras recebe a mesma resposta, sem precisar comentar."}
            </p>
          )}
          <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2.5">
            <span className="text-sm text-foreground">
              responder o comentário no post
            </span>
            <Toggle
          ...
          {publicReplyEnabled && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted">
                Uma resposta é escolhida ao acaso, para não ficar sempre igual.
              </p>
            </div>
          )}
        </Section>
        ...
        <Section title="A pessoa recebe">
          ...
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-foreground">
                exigir seguir a conta primeiro
              </span>
              <Toggle
                on={requireFollow}
            ...
            {requireFollow && (
              <div className="mt-3 space-y-2">
                ...
                />
                <p className="text-xs text-muted">
                  O link só vai depois do toque no botão e da confirmação do
                  Instagram. Se não der para confirmar, enviamos mesmo assim.
                </p>
              </div>
            )}
          </div>
        ...
        <Section title="Depois disso, recebe">
          <div className="rounded-lg border border-border p-3 space-y-2">
            <span className="text-sm text-foreground">um DM com link</span>
            <textarea
              value={dmMessage}
            ...
            <p className="text-xs text-muted">
              {"{link}"} coloca o link rastreado; {"{username}"} personaliza.
            </p>
          </div>
          <div className="mt-3 rounded-lg border border-border p-3">
            <div className="flex items-center justify-between">
              ...
              <div className="mt-3 space-y-2">
                ...
                <p className="text-xs text-muted">
                  {followUpDelayMinutes > 0
                    ? `Enviado ${followUpDelayMinutes} min depois do toque.`
                    : "Enviado logo depois do toque."}
                  {" {username}"} personaliza. Máximo 24 horas, dentro da janela
                  de mensagem do Instagram.
                </p>
              </div>
            )}
          </div>
